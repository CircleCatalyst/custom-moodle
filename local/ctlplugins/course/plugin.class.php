...
class moodlectl_plugin_course extends moodlectl_plugin_base {
    ...
    function help() {
     ...
     moodlectl list-courses",
			 'list-courses-participants' =>"  List all courses, WITH participants included (for Manual Sync):
			 moodlectl list-courses-participants",
    ...
    function execute($action, $options, $mode, $format) {
        ...
        switch ($action) {
            ...
            case 'list-courses':
                ...
                break;
			case 'list-courses-participants':
                // list all courses
                return moodlectl_plugin_course::list_all_courses_participants($format);
                break;
            case 'change-course':
                // update a course
...
/**
* 20110824 NEW BK: retrieve course details, including participants attached, used by Manual Moodle sync EXCLUSIVELY
*
* @param string $format the format of input/output
* @return array list of all the courses | or Exception()
* */
    static function list_all_courses_participants($format) {
        global $CFG, $DB;
...
* */
    static function change_course($data, $format) {
        ...
        // make sure that the shortname doesnt allready exist
		// BK: WTF? WHY? this generates an exception always if the shortname is passed unchanged and the course exists.. and that is exactly what we do in sms_moodle_messages_handler !!!
		// NOT what we want because we need info on the existing group passed back to KN, as the sms_moodle_messages_handler.php EXPECTS data back, not an exception!!!
		/*
